        ...
        private static void Main(string[] args)
        {
            ...
            switch (args.Length)
            {
                case 1 when args[0] == "-a":
                    var ttf = _taxonomyClient.GetFullTaxonomy(new TaxonomyVersion { Version = "1.0" });
                    ...
                    if (artifactSet && !string.IsNullOrEmpty(id))
                    {
                        switch (artifactType)
                        {
                            case ArtifactType.Base:
                                var b = _taxonomyClient.GetBaseArtifact(new ArtifactSymbol
                                {
                                    Id = id
                                });
                            ...
                            case ArtifactType.Behavior:
                                var behavior = _taxonomyClient.GetBehaviorArtifact(new ArtifactSymbol
                                {
                                    Id = id
                                });
                            ...
                            case ArtifactType.BehaviorGroup:
                                var behaviorGroup = _taxonomyClient.GetBehaviorGroupArtifact(new ArtifactSymbol
                                {
                                    Id = id
                                });
                            ...
                            case ArtifactType.PropertySet:
                                var propertySet = _taxonomyClient.GetPropertySetArtifact(new ArtifactSymbol
                                {
                                    Id = id
                                });
